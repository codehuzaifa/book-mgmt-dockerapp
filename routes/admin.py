...
def signup():
	...
	if request.method == 'POST':
		...
		if len(email)<1 or len(password)<1:
			return render_template('admin/signup.html', error="Email and password are required")

		new_admin = admin_manager.signup(email, hash(password))

		if new_admin == "already_exists":
			return render_template('admin/signup.html', error="Admin already exists with this email")

		return render_template('admin/signup.html', msg="Admin has been registered!")
